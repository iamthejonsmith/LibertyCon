...
@Component({
  selector: 'app-author-publisher',
  templateUrl: './author-publisher.component.html',
  styleUrls: ['./author-publisher.component.css']
})
export class AuthorPublisherComponent implements OnInit {
  authorPubs: any[] = new Array();
  ...
  setGuests() {
    for (let a = 0; a < (this.globals.guests).length; a++) {
      if (this.globals.guests[a].type === 'Author \/ Publisher') {
        this.authorPubs.push(this.globals.guests[a]);
      }
    }
  }
